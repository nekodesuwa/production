    ...
    <form method="POST" action="">
        <div class="payment-container">
            ...
            <div class="accordion-item">
                <input type="radio" name="payment_method" id="credit-card" value="クレジットカード" class="accordion-radio">
                ...
                <div class="accordion-content">
                    <p>登録済みのクレジットカードを選択してください。</p>
                    <?php if (!empty($cards)): ?>
                        <?php foreach ($cards as $index => $card): ?>
                            <input type="radio" name="card_id" value="<?php echo htmlspecialchars($card['card_id']); ?>" 
                                id="card-<?php echo htmlspecialchars($card['card_id']); ?>" class="card-radio"
                                <?php echo $index === 0 ? 'checked' : ''; ?>>
                            <label for="card-<?php echo htmlspecialchars($card['card_id']); ?>">****...<?php echo substr($card['card_number'], -4); ?></label><br>
                        <?php endforeach; ?>
                    <?php else: ?>
