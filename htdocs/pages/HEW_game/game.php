...
let playerBuff = false; // プレイヤーのカウンター用のフラグ
let playerMeditateBuff = false; // プレイヤーの瞑想効果用のフラグ
let enemyBuff = false; // 敵のカウンター用フラグ
let enemyMeditateBuff = false; // 敵の瞑想効果用フラグ
...
async function animateHPBar(target, fromHP, toHP, duration) {
    ...
    if (target.id === "player-hp-bar") {
        ...
        target.style.width = Math.floor(percentage) + "%"; // 幅を設定
        document.getElementById("player-hp-text").textContent = "HP: " + Math.floor(toHP); // HPテキスト更新
    } else if (target.id === "enemy-hp-bar") {
        const percentage = (toHP / enemyStats.maxHp) * 100; // 敵の最終HPを割合に
        target.style.width = Math.floor(percentage) + "%"; // 幅を設定
        document.getElementById("enemy-hp-text").textContent = "HP: " + Math.floor(toHP); // HPテキスト更新
    }
...
function checkGameOver() {
    if (enemyStats.hp <= 0) {
        ...
        //  勝利時に win_count を +1 する PHP を呼び出し
...
// チュートリアルのツールチップを制御するスクリプト（追加不要でも問題なし）
